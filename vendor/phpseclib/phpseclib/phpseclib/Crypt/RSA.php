...
class RSA extends AsymmetricKey
{
    ...
    {
		$dbg=false;
//		foreach(debug_backtrace() as $stack){if($stack['function']=='validateSignatureHelper') $dbg=true;}
		...
		if($dbg){echo("emsa_pss_verify::m:\n");$this->hex_dump($m);}
		if($dbg){echo("emsa_pss_verify::em:\n");$this->hex_dump($em);}


        $mHash = $this->hash->hash($m);
		if($dbg){echo("emsa_pss_verify::mHash:\n");$this->hex_dump($mHash);}
        if ($emLen < $this->hLen + $sLen + 2) {
            return false;
        }
    ...
    {		$dbg=false;
//		foreach(debug_backtrace() as $stack){if($stack['function']=='validateCountersignature') $dbg=true;}
...
//		foreach(debug_backtrace() as $stack){if($stack['function']=='validateSignatureHelper') $dbg=true;}
		...
		if($dbg){echo(__FUNCTION__."::m:\n");$this->hex_dump($m);}
		if($dbg){echo(__FUNCTION__."::s:\n");var_dump($s);}
		if($dbg){echo(__FUNCTION__."::m2:\n");var_dump($m2);}
		if($dbg){echo(__FUNCTION__."::em:\n");$this->hex_dump($em);}
        // If the encoding operation outputs "intended encoded message length too short," output "RSA modulus
        // too short" and stop.
        try {
            $em2 = $this->emsa_pkcs1_v1_5_encode($m, $this->k);
			if($dbg){echo(__FUNCTION__."::em2:\n");$this->hex_dump($em2);}
        } catch (\LengthException $e) {
            throw new \LengthException('RSA modulus too short');
        }
//		var_dump($this->hLen);
		if($dbg){echo(__FUNCTION__."::hash:");var_dump(bin2hex(substr($em2,-($this->hLen))));}
        // Compare
        return Strings::equals($em, $em2);
    }
	
	private function rsassa_pkcs1_v1_5_verify_raw($m, $s)
    {
        if (strlen($s) != $this->k) {
            return false;
        }
		...
		$hash = $this->hash->hash($m);
		$ps = str_repeat(chr(0xFF), $this->k - strlen($hash) - 3);
			
		$em2raw = "\0\1$ps\0$hash";
			
        // Compare
		return Strings::equals($em, $em2raw);
    }
	...
	{	
        ...
        if (Strings::shift($em, 2) != "\0\1") {
			if($dbg){echo(__FUNCTION__."::Strings::shift:\n");}
            return false;
        }

        $em = ltrim($em, "\xFF");
        if (Strings::shift($em) != "\0") {
			if($dbg){echo(__FUNCTION__."::ltrim:\n");}
            return false;
        }

        $decoded = ASN1::decodeBER($em);
        if (!is_array($decoded) || empty($decoded[0]) || strlen($em) > $decoded[0]['length']) {
			if($dbg){echo(__FUNCTION__."::decoderBER\n");}
            return false;
        }
    ...
    public function verify($message, $signature, $padding = self::PADDING_PSS)
    {
        ...
        switch ($padding) {
            ...
            case self::PADDING_PKCS1_RAW:
                return $this->rsassa_pkcs1_v1_5_verify_raw($message, $signature);
            //case self::PADDING_PSS:
            default:
