   ...
   const STRING_TYPE_SIZE = [
    ...
    private static function decode_ber($encoded, $start = 0, $encoded_pos = 0)
    {
        ...
        if ($tag == 0x1F) {
            ...
            do {
                $loop = ord($encoded[$encoded_pos]) >> 7;
                $tag <<= 7;
        ...
        $class = ($type >> 6) & 3;
//		echo(__LINE__." class:");var_dump($class);
//		echo(__LINE__." tag:");var_dump($tag);
//		echo(__LINE__." content:");var_dump($content);
        switch ($class) {
     ...
     * @access public
     */
    public static function asn1map($decoded, $mapping, /*$dbg=false,*/ $special = [])
    {
		$dbg=false;
		if($dbg)echo("=================asn1map:\n");
		if($dbg){echo(__LINE__.":");echo("find:".$mapping['type']."\n");}
		if($dbg)debug_print_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS);
        if (isset($mapping['explicit']) && isset($decoded['content']) && is_array($decoded['content'])) {
            $decoded = $decoded['content'][0];
        }
		
		if($dbg){echo(__LINE__.": map ");var_dump($mapping);}
		if($dbg){echo(__LINE__.": for ");var_dump($decoded);}
...
//					echo("=======decoded:\n");//echo(__LINE__.":");var_dump($decoded);echo("=======\n");
                    if($dbg){echo(__LINE__.": returning\n");}return new Element(substr(self::$encoded, $decoded['start'], $decoded['length']),$decoded['start'],$decoded['headerlength']);
                }
                $inmap = self::ANY_MAP[$intype];
                if (is_string($inmap)) {
                    if($dbg){echo(__LINE__.": returning\n");}return [$inmap => self::asn1map($decoded, ['type' => $intype] + $mapping,$dbg, $special)];
                }
                break;
            case $mapping['type'] == self::TYPE_CHOICE:
                foreach ($mapping['children'] as $key => $option) {
				if($dbg){echo(__LINE__.":");echo("choice key:".$key.":\n");}
				
				if($dbg){echo(__LINE__.":");var_dump($decoded);}
				if($dbg){echo(__LINE__.":");echo("option constant:".$option['constant'].":\n");}
				if($dbg){echo(__LINE__.":");echo("decoded constant:".$decoded['constant'].":\n");}
                    switch (true) {
                        case isset($option['constant']) && $option['constant'] == $decoded['constant']:
                        case !isset($option['constant']) && $option['type'] == $decoded['type']:
                            $value = self::asn1map($decoded, $option,$dbg, $special);
                            break;
                        case !isset($option['constant']) && $option['type'] == self::TYPE_CHOICE:
                            $v = self::asn1map($decoded, $option,$dbg, $special);
                            if (isset($v)) {
                                $value = $v;
                            }
                    }
                ...
                if($dbg){echo(__LINE__.": returning null\n");}return null;
		...
		if($dbg){echo(__LINE__.":");echo("found:".$decoded['type']."\n");}
...
//		echo("==============================================\n");
        ...
        switch ($decoded['type']) {
            case self::TYPE_SEQUENCE:
				if($dbg){echo(__LINE__.":");echo("sequence found, digging into children.\n");}
                $map = [];
                ...
                if (isset($mapping['min']) && isset($mapping['max'])) {
                    ...
                    foreach ($decoded['content'] as $content) {
                        if (($map[] = self::asn1map($content, $child,$dbg, $special)) === null) {
                           if($dbg){echo(__LINE__.": returning null\n");} return null;
                        }
                    }
				...
				if($dbg){echo(__LINE__.":");echo("next key:".$key.":\n");}
				if($dbg){echo(__LINE__.":");echo("next child:");var_dump($child);}

                    $maymatch = $i < $n; // Match only existing input
					//if($dbg){echo(__LINE__.":");var_dump("maymatch".$maymatch);}
                    if ($maymatch) {
                        $temp = $decoded['content'][$i];
						if($dbg){echo(__LINE__.":");var_dump($temp);}
                        if ($child['type'] != self::TYPE_CHOICE) {
                            // Get the mapping and input class & constant.
                            $childClass = $tempClass = self::CLASS_UNIVERSAL;
                            $constant = null;
                            if (isset($temp['constant'])) {
								if($dbg){echo(__LINE__."\n");}
							...
							if($dbg){echo(__LINE__.":");var_dump($constant);}
							if($dbg){echo(__LINE__.":");var_dump($temp['constant']);}
                            if (isset($constant) && isset($temp['constant'])) {
                                if($dbg){echo(" Can only match if constants and class match.\n");}
								//echo(__LINE__.":");var_dump(isset($constant), isset($temp['constant']));
                                $maymatch = $constant == $temp['constant'] && $childClass == $tempClass;
                            } else {
                                if($dbg){echo(" Can only match if no constant expected and type matches or is generic.\n");}
								//echo(__LINE__.":");var_dump("array_search:",$child['type'], $temp['type'],(!isset($child['constant'])),array_search($child['type'], [$temp['type'], self::TYPE_ANY, self::TYPE_CHOICE]));
                                $maymatch = !isset($child['constant']) && !isset($temp['constant']) && array_search($child['type'], [$temp['type'], self::TYPE_ANY, self::TYPE_CHOICE]) !== false;
                            }
                        }
                    }
//					if($dbg){echo(__LINE__.":");var_dump($temp);}
                    ...
                    if ($maymatch) {
                        // Attempt submapping.
                        $candidate = self::asn1map($temp, $child, $dbg, $special);
...
//							case ASN1::TYPE_BOOLEAN:
						...
						if($dbg){echo(__LINE__.":");var_dump("Syntax error.");}
                        if($dbg){echo(__LINE__.": returning null\n");}return $dbg?$map:null;//($dbg?$map:null); // Syntax error.
                    }
                }

                // Fail mapping if all input items have not been consumed.
                if($dbg){echo(__LINE__.": returning\n");}return $i < $n ? ($dbg?$map:null): $map;
