...
//				'1.3.132.0.23' => '23',
    ...
    public function loadX509($cert, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        $key = $this->reformatKey($x509['tbsCertificate']['subjectPublicKeyInfo']['algorithm'], $key);
            ...
            case 'rsaEncryption':
                ...
                switch ($signatureAlgorithm) {
                    ...
                    case 'sha512WithRSAEncryption':
                        ...
                        break;
                    default:
                        throw new UnsupportedAlgorithmException('Signature algorithm unsupported:'.$signatureAlgorithm);
                }
                break;
			case 'id-ecPublicKey':
				$der = (base64_decode(explode("-----",$publicKey)[2]));
				$pubKeyInfo = (ASN1::asn1map(ASN1::decodeBER($der)[0],Maps\SubjectPublicKeyInfo::MAP));
				$curve = ($pubKeyInfo['algorithm']['parameters']['objectIdentifier']);
...
//				var_dump($key);
				...
				$hash = $hasher->makeHash($signatureSubject, $generator);
				$signer = new \Mdanter\Ecc\Crypto\Signature\Signer($adapter);
				$check = $signer->verify($key, $sig, $hash);
				//var_dump($check);
		
				//var_dump($signatureAlgorithm,($publicKey),base64_encode($signature),base64_encode($signatureSubject));
				return $check;
            default:

                throw new UnsupportedAlgorithmException('Public key algorithm unsupported:'.$publicKeyAlgorithm);
        }
    ...
    private function reformatKey($algorithm, $key)
    {
        switch ($algorithm['algorithm']) {
            ...
            case 'id-ecPublicKey':
				...
				$der = ASN1::encodeDER($ecPublicKey,Maps\SubjectPublicKeyInfo::MAP);
				$base64 = base64_encode($der);
				//var_dump($ecPublicKey);
				//hex_dump($key);
     ...
     * @return array|false
     */
    protected function &extensions(&$root, $path = null, $create = false)
    {
        if (!isset($root)) {
